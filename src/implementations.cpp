...
void apply_thread_x(StateVector& statevector, const std::vector<int>& qubits, const int& n_qubits, const uint16_t& thread)
{
    ...
    uint16_t k = std::max(0, qubits[0] - n_qubits + n_threads_exponent + 1);
    uint64_t position = (1ULL << (n_qubits - n_threads_exponent)) * (thread  - (thread % (1ULL << k)));
    uint64_t counter = 0;
    while (counter < (1ULL << (n_qubits - n_threads_exponent - 1))) {
        aux = statevector[position];
        statevector[position] = statevector[flipbit(position, qubits[0])];
        statevector[flipbit(position, qubits[0])] = aux;
        (counter % (1ULL << qubits[0])) == ((1ULL << qubits[0]) - 1) ? position = position + ((1ULL << qubits[0]) + 1) : position++;
        counter++;
    }
}

meas_out cunqa_apply_measure(StateVector& statevector, const std::vector<int> qubits, const int& n_qubits, bool threads)
{
    meas_out output;
    bool zero;
    double prob_0 = 0;
...
void cunqa_apply_cx(StateVector& statevector, const std::vector<int> qubits, const int& n_qubits, bool threads)
{
    ...
    for (uint64_t j = (1ULL << qubits[0]); j < statevector.size(); j = j + (1ULL << (qubits[0] + 1))) {
        for (uint64_t i = j - (1ULL << qubits[0]); i < j; i++) {
            zero = is_zero(flipbit(i, qubits[0]), qubits[1]);
            if (zero) {
                aux_01 = statevector[flipbit(i, qubits[0])];
                statevector[flipbit(i, qubits[0])] = statevector[flipbit(flipbit(i, qubits[0]), qubits[1])];
                statevector[flipbit(flipbit(i, qubits[0]), qubits[1])] = aux_01;
            }
        }
    }
}
...
void cunqa_apply_cy(StateVector& statevector, const std::vector<int> qubits, const int& n_qubits, bool threads)
{
    ...
    for (uint64_t j = (1ULL << qubits[0]); j < statevector.size(); j = j + (1ULL << (qubits[0] + 1))) {
        for (uint64_t i = j - (1ULL << qubits[0]); i < j; i++) {
            ...
            if (zero) {
                aux_01 = statevector[flipbit(i, qubits[0])];
                statevector[flipbit(i, qubits[0])] = imag * statevector[flipbit(flipbit(i, qubits[0]), qubits[1])];
            } else {
                statevector[flipbit(i, qubits[0])] = -imag * aux_01;
            }
        }
    }
}
    ...
    for (uint64_t j = (1ULL << qubits[0]); j < statevector.size(); j = j + (1ULL << (qubits[0] + 1))) {
        for (uint64_t i = j - (1ULL << qubits[0]); i < j; i++) {
            ...
            if (zero) {
                ...
                aux_10 = statevector[i];
                statevector[i] = imag * inverse_sqrt_2 * aux_01 + inverse_sqrt_2 * statevector[flipbit(i, qubits[0])];
                statevector[flipbit(i, qubits[0])] = -imag * inverse_sqrt_2 * aux_00 + inverse_sqrt_2 * aux_10;
            }
        }
    }
}
...
{
    meas_out meas = cunqa_apply_measure(statevector, {qubits[0]}, n_qubits, threads);
    if (meas.measure == 1) {
...
void cunqa_apply_crx(StateVector& statevector, const std::vector<int> qubits, const Params& param, const int& n_qubits, bool threads)
{
    ...
    for (uint64_t j = (1ULL << qubits[0]); j < statevector.size(); j = j + (1ULL << (qubits[0] + 1))) {
        for (uint64_t i = j - (1ULL << qubits[0]); i < j; i++) {
            ...
            if (zero) {
                aux_01 = statevector[flipbit(i, qubits[0])];
                statevector[flipbit(i, qubits[0])] = cos * statevector[flipbit(i, qubits[0])] - imag * sin * statevector[flipbit(flipbit(i, qubits[0]), qubits[1])];
            } else {
                statevector[flipbit(i, qubits[0])] = cos * statevector[flipbit(i, qubits[0])] - imag * sin * aux_01;
            }
        }
    }
}
...
void cunqa_apply_cry(StateVector& statevector, const std::vector<int> qubits, const Params& param, const int& n_qubits, bool threads)
{
    ...
    for (uint64_t j = (1ULL << qubits[0]); j < statevector.size(); j = j + (1ULL << (qubits[0] + 1))) {
        for (uint64_t i = j - (1ULL << qubits[0]); i < j; i++) {
            ...
            if (zero) {
                aux_01 = statevector[flipbit(i, qubits[0])];
                statevector[flipbit(i, qubits[0])] = cos * statevector[flipbit(i, qubits[0])] + sin * statevector[flipbit(flipbit(i, qubits[0]), qubits[1])];
            } else {
                statevector[flipbit(i, qubits[0])] = cos * statevector[flipbit(i, qubits[0])] - sin * aux_01;
            }
        }
    }
}
